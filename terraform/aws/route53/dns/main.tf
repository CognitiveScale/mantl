...
resource "aws_route53_record" "dns-control" {
  ...
  records = ["${element(split(",", var.control_eips), count.index)}"]
...
resource "aws_route53_record" "dns-edge" {
  ...
  zone_id = "${var.hosted_zone_id}"
  records = ["${element(split(",", var.edge_eips), count.index)}"]
...
resource "aws_route53_record" "dns-worker" {
  ...
  records = ["${element(split(",", var.worker_eips), count.index)}"]
  type = "A"
  ttl = 60
}
...
resource "aws_route53_record" "dns-control-group" {
  ...
  records = ["${split(",", var.control_eips)}"]
  type = "A"
  ttl = 60
}
